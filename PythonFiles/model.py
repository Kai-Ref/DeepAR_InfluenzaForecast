...
def train_test_split(config, df, with_features=False):
    """
    This function splits the input DataFrame into train and test sets and formats them into a PandasDataset 
    with or without features based on the specified options.
...
def process_R_results(config, results_df, influenza_df, validation=False):
    '''
    Process the results_df (produced by the hhh4 model in [DeepAR_InfluenzaForecast/R]) by adding a date column and the corresponding true values.
...
def evaluate_R_forecasts(config, df_dict, locations, processed_df):
    """
    Evaluate the forecasts generated by the R model.
