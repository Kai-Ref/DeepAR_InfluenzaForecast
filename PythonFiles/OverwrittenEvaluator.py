# Overwritten version of the Evaluator, as we want to add a custom function
# Note that the function custom_eval_fn is (up to now) only applicable to the mean or median of the forecasts
# However we want to compare the Coverage ("<=") with the Strict Coverage ("<") for various quantile levels
...
# functions that were already implemented in the class
...
# Addition of the strict_coverage metric
def strict_coverage(target: np.ndarray, forecast: np.ndarray) -> float:
    r"""
    .. math::
...
# Only made minor changes to involve the strict_coverage metric 
class Evaluator:
    """
    ...
    def get_metrics_per_ts(
        ...
        for quantile in self.quantiles:
            ...
            metrics[f"StrictCoverage[{quantile}]"] = strict_coverage(
                pred_target, forecast_quantile
            )
    ...
    def get_aggregate_metrics(
        ...
        for quantile in self.quantiles:
            ...
            agg_funs[f"StrictCoverage[{quantile}]"] = "mean"
        ...
        totals["MAE_StrictCoverage"] = np.mean(
            [
                np.abs(totals[f"StrictCoverage[{quantile}]"] - np.array([q.value]))
                for q in self.quantiles
            ]
        )
